...
function Banner({ message, type }) {
  ...
  const cfg = {
    ...
    warning: { cls: "bg-amber-50 border-amber-200 text-amber-700", Icon: Clock },
...
export default function DriverLoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 2. Look up their driver profile
      ...
      if (profileError || !profile) {
        ...
        throw new Error("No driver profile found for this account. Please register first.")
      }

      // 3. Check approval status
      if (!profile.is_approved) {
        setStatus({
          message: "Your account is still pending admin approval. You will be notified when approved.",
          type: "warning",
        })
        setIsLoading(false)
        return
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <header className="shrink-0 bg-white border-b border-slate-200 px-6 py-3.5 flex items-center justify-between">
        ...
        <Link href="/register/driver" className="text-xs font-semibold text-slate-400 hover:text-slate-700 transition-colors">
          ← Register instead
        </Link>
      </header>
          ...
          <div className="bg-white border border-emerald-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="bg-gradient-to-br from-emerald-800 to-emerald-700 px-7 py-6">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold text-emerald-200 uppercase tracking-widest">Driver Portal</span>
              </div>
              <h1 className="text-xl font-bold text-white">Sign In to Your Account</h1>
              <p className="text-emerald-100 text-sm mt-1">
                Enter your email and password to access the Driver Terminal.
              </p>
            </div>
            ...
            <form onSubmit={handleLogin} className="px-7 py-7 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="w-full border border-slate-200 bg-white rounded-xl py-2.5 pl-10 pr-3.5 text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-700/40 focus:border-emerald-700 transition-all"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full h-11 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading
